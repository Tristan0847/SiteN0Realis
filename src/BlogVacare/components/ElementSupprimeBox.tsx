...
import {ElementSupprime} from "@BlogsFront/model/Blog";
...
export default function ElementSupprimeBox({type, donnees}: SuppressionBoxProps) {
    ...
    return (
        <div>
            <div
                className={type == "message" ? styles.elementSupprTitreMessage : styles.elementSupprTitre}>⚠️ {upperType} supprimé
                — <p className={styles.elementSupprTitreLien} onClick={() => setDialogBox(true)}>Voir les détails</p>
            </div>
            <Dialog open={dialogBoxOuverte} onClose={() => setDialogBox(false)}
                    className={styles.elementSupprDialogBox}>
                <DialogPanel className={styles.elementSupprDialogPanel}>
                    ...
                    <div className="mb-2">
                        <span className={styles.elementSupprDialogContenu}>Raison :</span> {donnees.raison_suppression}
                    </div>
                    <div className="mb-2">
                        <span className={styles.elementSupprDialogContenu}>Date :</span> {new Date(donnees.date_suppression).toLocaleString('fr-FR', {timeZone: 'UTC'})}
                    </div>
                    <div>
                        <span className={styles.elementSupprDialogContenu}>Auteur :</span> {donnees.nom_utilisateur}
                    </div>
