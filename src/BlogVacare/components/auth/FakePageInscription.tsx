                    ...
                    {lignes.map((liens, indexLigne) => (
                        <tr key={indexLigne}>
                            {liens.map((lien, indexColonne) => (
                                <td key={indexColonne}
                                    className="px-2 py-1 text-center hover:underline hover:scale-[1.2] transition">
                                    {lien}
                                </td>
                            ))}
                        </tr>
        ...
        return (
            <RatsConteneur className="relative z-0">
                <div className="absolute inset-0 -z-10 w-full h-full opacity-25">
                    <Image className="opacity-5" src="/assets/Rats/N0Records.png" alt="N0Records" layout="fill"
                           objectFit="cover"/>
                </div>
                <h2 className="text-center my-auto text-white text-3xl p-4 relative z-10">Les N0Records seront délivrés
                    en temps voulu.</h2>
            </RatsConteneur>
        );
    }
