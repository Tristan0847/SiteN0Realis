...
interface SuppressionBoxProps {
    ...
    suppressionHandler: (id: number, raisonSuppression: string, cache: boolean) => Promise<void>;
            ...
            <div className={styles.supprFormConteneur}>
                <DialogPanel className={styles.supprFormPanel}>
                    <div className={styles.supprFormContenu}>
                        ...
                        <Description className={styles.supprFormDesc}>Cette action supprimera le {type}. Les
                            utilisateurs non-admins n&apos;y auront plus accès.<br/><br/>Voulez-vous vraiment le
                        ...
                        <div className="space-y-3">
                            <div>
                                <label htmlFor="raison" className={styles.supprFormRaisonLabel}>Raison de la
                                    suppression</label>
                                <textarea id="raison" value={raisonSuppression}
                                          onChange={(e) => setRaisonSuppression(e.target.value)}
                                          className={styles.supprFormRaison} rows={3}
                                          placeholder="Indiquez la raison de cette suppression..."
                                          disabled={chargement}/>
                            </div>

                            <div className="flex items-center gap-2">
                                <input type="checkbox" id="cache" checked={cache}
                                       onChange={(e) => setCache(e.target.checked)} className={styles.supprFormCache}
                                       disabled={chargement}/>
                                <label htmlFor="cache" className={styles.supprFormCacheLabel}>Cochez cette case si vous
                                    souhaitez que le {type} soit caché après suppression.</label>
                            </div>
                        </div>

                        <div className="flex w-full pt-2 gap-4 justify-center">
                            <button onClick={() => fermetureFormulaire()}
                                    className={styles.supprFormBtnAnnuler}>Annuler
                            </button>
                            <button onClick={() => confirmationHandler()}
                                    className={styles.supprFormBtnConfirmer}>Supprimer
                            </button>
                        </div>
                    </div>
                </DialogPanel>
