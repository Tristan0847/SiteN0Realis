...
export async function generateMetadata({params}: PageProps): Promise<Metadata> {
    ...
    return {
        title: `${parametres.slugDossier} - Forum de SuperFlashAtomicMan et Vince`,
    };
}
...
export default async function Page({params}: PageProps) {
    ...
    const blogsSerialises = await getBlogsPrecharges(slugDossier, "old");

    // La page transmet l'idDossier au composant client (vue)
    return <PageBlogsClient slugDossier={slugDossier}
                            blogsPrecharges={(blogsSerialises?.length > 0) ? blogsSerialises : undefined}/>;
}
